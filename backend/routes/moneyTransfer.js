...
money.post('/addTransaction', verifyJWT, async (req, res) => {
    try {
        const authToken = req.headers.authorization;
        const decoded = jwt.decode(authToken);
        const role = decoded.role;
        ...
        if (!caseId || !amount) {
            return res.status(400).json({ msg: "Missing caseId or amount" });
        }
        ...
        console.error(err);
        res.status(500).json({
            msg: "Something went wrong while adding transaction to the blockchain",
            err
        });
    }
});
